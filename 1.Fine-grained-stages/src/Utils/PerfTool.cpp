...
ThreadPerf::ThreadPerf(int cpu) {
  ...
  //ll
  //pairs.push_back(PerfPair(COUNT_HW_CACHE_L1D_LOADS,"L1DLoad"));
  pairs.push_back(PerfPair(COUNT_HW_CACHE_L1I_LOADS_MISSES, "L1ILoadMiss"));
  // pairs.push_back(PerfPair(COUNT_HW_CACHE_L1D_STORES,"L1DStore"));
//pairs.push_back(PerfPair(COUNT_HW_CACHE_L1D_STORES_MISSES,"L1DStoreMiss"));

  //pairs.push_back(PerfPair(COUNT_HW_CACHE_L1I_LOADS,"L1ILoad"));
  pairs.push_back(PerfPair(COUNT_HW_CACHE_L1D_LOADS_MISSES, "L1DLoadMiss"));

  /* pairs.push_back(PerfPair(COUNT_HW_CACHE_LL_LOADS,"LLLoad"))d
 pairs.push_back(PerfPair(COUNT_HW_CACHE_LL_LOADS_MISSES,"LLLoadMiss"));
    pairs.push_back(PerfPair(COUNT_HW_CACHE_LL_STORES,"LLtore"));
 pairs.push_back(PerfPair(COUNT_HW_CACHE_LL_STORES_MISSES,"LLStoreMiss"));

  pairs.push_back(PerfPair(COUNT_HW_CACHE_ITLB_LOADS,"ITLBLoad"));*/
  pairs.push_back(PerfPair(COUNT_HW_CACHE_ITLB_LOADS_MISSES, "ITLBLoadMiss"));



//pairs.push_back(PerfPair(COUNT_HW_CACHE_DTLB_LOADS,"DTLBLoad"));
  pairs.push_back(PerfPair(COUNT_HW_CACHE_DTLB_LOADS_MISSES, "DTLBLoadMiss"));


//pairs.push_back(PerfPair(COUNT_HW_CACHE_DTLB_STORES,"DTLBStore"));
//pairs.push_back(PerfPair(COUNT_HW_CACHE_DTLB_STORES_MISSES,"DTLBStoreMiss"));
...
uint64_t ThreadPerf::getResultById(size_t idx) {
  if (idx > pairs.size()) {
    return 0;
  }
  size_t ch = pairs[idx].ref;
...
void ThreadPerf::setUpTransitVec(void) {
  memAccessTransitVec = vector<double>();
  memAccessTransitVec.push_back(1.0);
  memAccessTransitVec.push_back(testMem<uint64_t>(10000));
  memAccessTransitVec.push_back(testMem<uint16_t>(10000));
  memAccessTransitVec.push_back(testMem<uint8_t>(10000));
}
double ThreadPerf::getIpm(length_t mtimes) {
  double i = getResultByName("instructions");
  double m = 0;
  m += mtimes;
  return i / m;
}