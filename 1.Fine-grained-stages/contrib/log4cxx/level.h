...
level set.
*/
class LOG4CXX_EXPORT Level
    : public helpers::ObjectImpl {
 public:
  class LOG4CXX_EXPORT LevelClass
      : public helpers::Class {
   public:
    ...
    virtual LevelPtr toLevel(const LogString &sArg) const { return Level::toLevelLS(sArg); }

    virtual LevelPtr toLevel(int val) const { return Level::toLevel(val); }
  };
...
*/
  ...
  inline bool operator==(const Level &level1) const { return (this->level == level1.level); }
