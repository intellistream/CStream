...
static double tmdiff(tm_t start, tm_t stop) {
  return (stop.tv_sec - start.tv_sec) + (double) (stop.tv_nsec - start.tv_nsec) / 1e9f;
}
static tm_t tminit() {
  tm_t t0 = tmtime(), t;
  while (!tmdiff(t = tmtime(), t0)) {};
  return t;
}
...
static void tm_init(int _tm_Rep, int _tm_verbose) {
  tm_verbose = _tm_verbose;
...
static unsigned argtoi(char *s, unsigned def) {
  ...
  switch (*p) {
    ...
    case 'B': return n;
      break;
    case 'b': def = 0;
    default: if (!def) return n >= 32 ? 0xffffffffu : (1u << n);
      f = def;
  }
  return n * f;
}
