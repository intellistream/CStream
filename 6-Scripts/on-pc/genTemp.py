...
def writeTemplateCsv(lat,eng):
    ...
    for i in range(len(lat)):
        if (lat[i]>ts):
            ts=lat[i]
            idxMaxLat=i
    acam_eng=eng[idxMinEng]
    acam_lat=lat[idxMinEng]
    ...
    with open("plans_temp.csv","w") as csvfile: 
        ...
        writer.writerow(["ACAM",acam_eng,acam_lat,0,0])
        ...
        writer.writerow(["NS",acam_eng,acam_lat,0,0])
def writeTemplate2ndCsv(lat,eng):
    ...
    acam_eng=eng[idxMinEng2]
    acam_lat=lat[idxMinEng2]
    #print(lat2Idex,acam_eng,acam_lat)
